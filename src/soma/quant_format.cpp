...
Status dequantize_tensor(const QTensor& src, std::span<float> out) {
    const std::size_t n = static_cast<std::size_t>(src.rows) * src.cols;
    if (out.size() < n) {
        return {StatusCode::InvalidArgument, "dequantize_tensor: destination too small"};
    }
    const auto g = src.group;
    const auto gb = group_bytes(src.dtype, g);
    const auto per_row = (static_cast<std::size_t>(src.cols) / g) * gb;

    for (std::uint32_t r = 0; r < src.rows; ++r) {
        const std::byte* p = src.data.data() + static_cast<std::size_t>(r) * per_row;
        float* dst = out.data() + static_cast<std::size_t>(r) * src.cols;
        for (std::uint32_t c = 0; c < src.cols; c += g) {
            ...
            default:
                return {StatusCode::Unsupported,
                        std::string("dequantize: unsupported dtype ") + to_string(src.dtype)};
            }
            p += gb;
        }
    }
