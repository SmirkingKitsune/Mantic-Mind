...
void fake_fp8(std::span<float> x, std::uint32_t group = 64) noexcept {
    for (std::size_t at = 0; at < x.size(); at += group) {
        ...
        for (std::size_t i = 0; i < n; ++i)
            amax = std::max(amax, std::abs(x[at + i]));
...
void semantic_matvec(const F32Model& model,
                     const WeightRef& weight,
                     std::span<const float> input,
                     std::span<float> output) {
    std::vector<float> qinput(input.begin(), input.end());
    if (model.arch.attention.compressed.semantic_fp8_quant_dequant) fake_fp8(qinput, 128);
    matvec(weight, qinput, output);
    bf16_round(output);
}

void semantic_matmul(const F32Model& model,
                     const WeightRef& weight,
                     std::span<const float> input,
                     std::uint32_t rows,
                     std::span<float> output) {
    ...
    if (model.arch.attention.compressed.semantic_fp8_quant_dequant) {
        ...
        for (std::uint32_t row = 0; row < rows; ++row)
            fake_fp8(std::span<float>(qinput).subspan(static_cast<std::size_t>(row) * width, width),
                     128);
    }
...
StatusCode bind_model(F32Model& model, const std::string&) noexcept {
    ...
    if (!bind_weight(model, "model.dspark.main_proj.weight", p->main_proj) ||
        ...
        !bind_f32(model, "model.dspark.hc_head_scale", p->hc_head_scale))
        return fail();
    ...
    const auto sz = [&](std::uint32_t rows, std::uint32_t cols, TensorRole role) {
        ...
        return static_cast<std::uint32_t>(
            quantized_tensor_bytes(s.dtype, rows, cols, s.group ? s.group : kDefaultGroup));
    };
...
StatusCode open_sequence(const F32Model& model, std::uint32_t, ArchLayerPayload& out) noexcept {
    auto* s = new State();
...
void hc_begin(const ArchIr& a, const float* hidden, std::uint32_t rows, HcState& s) {
    s.rows = rows;
    s.d = a.topology.d_model;
    s.hc = a.hyper_connections.multiplier;
    ...
    for (std::uint32_t t = 0; t < rows; ++t)
        for (std::uint32_t h = 0; h < s.hc; ++h)
            std::copy_n(hidden + static_cast<std::size_t>(t) * s.d,
                        s.d,
                        s.streams.data() + (static_cast<std::size_t>(t) * s.hc + h) * s.d);
}

bool hc_pre(const ArchIr& a,
    ...
    if (scale.size() < 3 || fn.size() != static_cast<std::size_t>(mix) * flat || base.size() != mix)
        return false;
    ...
    for (std::uint32_t t = 0; t < s.rows; ++t) {
        ...
        for (std::uint32_t m = 0; m < mix; ++m) {
            ...
            for (std::uint32_t i = 0; i < flat; ++i)
                v += static_cast<double>(row[i]) * streams[i];
        ...
        for (std::uint32_t r = 0; r < hc; ++r) {
            ...
            for (std::uint32_t c = 0; c < hc; ++c)
                matrix[r * hc + c] =
                    std::exp(matrix[r * hc + c] - mx) / sum + a.hyper_connections.eps;
        }
        const auto cols = [&]() {
            for (std::uint32_t c = 0; c < hc; ++c) {
        ...
        cols();
        for (std::uint32_t i = 1; i < a.hyper_connections.sinkhorn_iters; ++i) {
            rows();
            cols();
        }
    ...
    for (std::uint32_t t = 0; t < s.rows; ++t) {
        ...
        for (std::uint32_t o = 0; o < s.hc; ++o) {
            ...
            for (std::uint32_t k = 0; k < s.d; ++k) {
                double v =
                    static_cast<double>(post[o]) * branch[static_cast<std::size_t>(t) * s.d + k];
        ...
        std::copy_n(next.data() + static_cast<std::size_t>(t) * s.hc * s.d,
                    s.d,
...
void project_kv(
    const F32Model& model, const Layer& l, const float* x, std::uint32_t pos, std::span<float> kv) {
...
StatusCode observe_target(const F32Model& model,
    ...
    if (p == nullptr || s == nullptr || taps.layers.size() != p->layers.size() ||
        first + count > taps.n_rows)
        return StatusCode::InvalidArgument;
    const auto d = model.d_model(), naux = static_cast<std::uint32_t>(taps.layers.size());
    std::vector<float> cat(static_cast<std::size_t>(naux) * d), main(d),
        kv(model.arch.attention.head_dim);
    for (std::uint32_t r = 0; r < count; ++r) {
        for (std::uint32_t a = 0; a < naux; ++a) {
            const auto src = taps.layer(a);
            std::copy_n(src.data() + static_cast<std::size_t>(first + r) * d,
                        d,
        ...
        for (std::uint32_t stage = 0; stage < p->layers.size(); ++stage) {
            ...
            auto* dst = s->context_kv.data() +
                        (static_cast<std::size_t>(stage) * model.arch.attention.sliding_window +
                         pos % model.arch.attention.sliding_window) *
                            model.arch.attention.head_dim;
            for (std::uint32_t k = 0; k < kv.size(); ++k)
                dst[k] = to_bf16(kv[k]);
        }
...
void draft_attention(const F32Model& model,
                     const Layer& l,
                     const State& s,
                     std::uint32_t stage,
                     std::uint32_t start,
    ...
    std::vector<float> qlow(static_cast<std::size_t>(rows) * qr),
        q(static_cast<std::size_t>(rows) * H * D);
    std::vector<float> qkv(static_cast<std::size_t>(rows) * D);
    semantic_matmul(
        model, l.wq_a, std::span<const float>(x, static_cast<std::size_t>(rows) * d), rows, qlow);
    ...
    for (std::uint32_t t = 0; t < rows; ++t) {
        for (std::uint32_t h = 0; h < H; ++h) {
            ...
            for (std::uint32_t pos = ctx_first; pos < start; ++pos, ++at) {
                const auto* src =
                    s.context_kv.data() + (static_cast<std::size_t>(stage) * W + pos % W) * D;
                for (std::uint32_t k = 0; k < D; ++k)
                    key[k] = from_bf16(src[k]);
            ...
            for (std::uint32_t j = 0; j < rows; ++j, ++at) {
                ...
                logits[at] =
                    f32::dot(std::span<const float>(qh, D), std::span<const float>(k, D), D) *
                    scale;
                mx = std::max(mx, logits[at]);
            }
            ...
            std::copy_n(oh, D, sparse_heads.data() + (static_cast<std::size_t>(t) * H + h) * D);
    ...
    bf16_round(low);
    semantic_matmul(
        model, l.wo_b, low, rows, std::span<float>(out, static_cast<std::size_t>(rows) * d));
}

bool expert(const F32Model& model,
            Payload& p,
            std::uint32_t stage,
            ExpertId id,
            const float* x,
    ...
    if (bytes.size() < static_cast<std::size_t>(p.gate_bytes) + p.up_bytes + p.down_bytes)
        return false;
    const auto& q = model.quant_map;
    const auto fi = model.arch.ffn.expert_intermediate, d = model.d_model();
    const auto make =
        [&](std::size_t at, std::size_t n, TensorRole role, std::uint32_t r, std::uint32_t c) {
            const auto& spec = q.for_role(role);
            return WeightRef::from_quantized_bytes(
                bytes.subspan(at, n), spec.dtype, spec.group ? spec.group : kDefaultGroup, r, c);
        };
    ...
    const auto down = make(p.gate_bytes + p.up_bytes, p.down_bytes, TensorRole::ExpertDown, d, fi);
    ...
    if (limit > 0.0f)
        for (std::uint32_t i = 0; i < fi; ++i) {
            g[i] = std::min(g[i], limit);
            u[i] = std::clamp(u[i], -limit, limit);
        }
    f32::swiglu(g, u, fi, a);
    bf16_round(a);
...
bool draft_ffn(const F32Model& model,
               Payload& p,
               const Layer& l,
               std::uint32_t stage,
               const float* x,
    ...
    std::vector<float> g(model.arch.ffn.shared_intermediate), u(model.arch.ffn.shared_intermediate),
    ...
    for (std::uint32_t t = 0; t < rows; ++t) {
        ...
        for (std::uint32_t e = 0; e < E; ++e) {
            original[e] =
                std::sqrt(std::log1p(std::exp(-std::abs(logits[e]))) + std::max(logits[e], 0.0f));
    ...
    for (std::uint32_t t = 0; t < s.rows; ++t) {
        ...
        for (std::uint32_t h = 0; h < hc; ++h) {
            ...
            for (std::uint32_t i = 0; i < flat; ++i)
                v += static_cast<double>(row[i]) * streams[i];
            const float w =
                sigmoid(static_cast<float>(v) * rs * p.hc_head_scale[0] + p.hc_head_base[h]) +
                model.arch.hyper_connections.eps;
            for (std::uint32_t k = 0; k < d; ++k)
                dst[k] += w * streams[static_cast<std::size_t>(h) * d + k];
        }
...
StatusCode propose(const F32Model& model,
                   ...
                   float threshold,
                   SpeculativeProposal& out) noexcept {
    auto* p = payload.as<Payload>();
    auto* s = state.as<State>();
    if (p == nullptr || s == nullptr || !p->runtime_open || max_tokens == 0)
        return StatusCode::InvalidArgument;
    ...
    std::vector<TokenId> ids(B, model.arch.speculative.noise_token_id);
    ids[0] = anchor;
    std::vector<float> hidden(static_cast<std::size_t>(B) * d);
    for (std::uint32_t t = 0; t < B; ++t) {
        auto row = row_block(model.embed, ids[t], 1);
        if (dequantize(row, std::span<float>(hidden).subspan(static_cast<std::size_t>(t) * d, d))
                .code() != StatusCode::Ok)
    ...
    HcState hc;
    hc_begin(model.arch, hidden.data(), B, hc);
    ...
    for (std::uint32_t stage = 0; stage < p->layers.size(); ++stage) {
        ...
        for (std::uint32_t t = 0; t < B; ++t)
            f32::rmsnorm_into(
                std::span<const float>(hidden).subspan(static_cast<std::size_t>(t) * d, d),
                l.input_norm,
                d,
        ...
        draft_attention(model, l, *s, stage, s->length, normed.data(), B, branch.data(), out.sink);
    ...
    out.tokens.reserve(B);
    out.confidence.reserve(B);
    ...
    for (std::uint32_t t = 0; t < B; ++t) {
        ...
        std::copy(
            logits.begin(), logits.end(), base_logits.begin() + static_cast<std::size_t>(t) * V);
        ...
        for (std::uint32_t v = 0; v < V; ++v)
            logits[v] += bias[v];
        std::copy(
            logits.begin(), logits.end(), final_logits.begin() + static_cast<std::size_t>(t) * V);
        TokenId best = 0;
        for (TokenId v = 1; v < V; ++v)
        ...
        out.confidence.push_back(confidence);
        prev = best;
    }
...
Status
serialize_state(const F32Model& model, const ArchLayerPayload& state, std::vector<std::byte>& out) {
    ...
    out.resize(sizeof(std::uint32_t) * 2 + static_cast<std::size_t>(live) * stages * width * 2);
    ...
    for (std::uint32_t stage = 0; stage < stages; ++stage)
        for (std::uint32_t pos = s->length - live; pos < s->length; ++pos) {
            const auto* src =
                s->context_kv.data() + (static_cast<std::size_t>(stage) * W + pos % W) * width;
...
Status
restore_state(const F32Model& model, std::span<const std::byte> payload, ArchLayerPayload& state) {
    auto* s = state.as<State>();
    ...
    const auto stages = model.arch.speculative.n_layers;
    const auto want =
        sizeof(std::uint32_t) * 2 + static_cast<std::size_t>(live) * stages * width * 2;
    ...
    const auto* src =
        reinterpret_cast<const std::uint16_t*>(payload.data() + sizeof(std::uint32_t) * 2);
    for (std::uint32_t stage = 0; stage < stages; ++stage)
