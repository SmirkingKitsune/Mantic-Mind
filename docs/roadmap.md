...
| D43 | **`soma_engine_g5` failed once in a full-suite run and has not reproduced.** The captured log shows a supervised engine process crashing (`EngineSupervisor: engine <uuid> crashed with ...`) rather than an assertion, which puts it in the same family as D1 — a process/transport flake on Windows — though it is a different test. **Not caused by the indexer change**: engine_g5 runs `qwen3_moe`, a GQA model that never enters the MLA/DSA path that changed. It passed alone (15.1 s) and in two subsequent full-suite runs. Recorded on ONE observation, deliberately: the alternative is the next person seeing it once with no context, which is how a real intermittent bug gets dismissed as noise. The failing assertion was not captured before LastTest.log was overwritten. | indexer scoring | Low — one observation, no reproduction, and the exercised path is untouched by recent work. |
