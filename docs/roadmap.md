...
|---|---|---|---|
| D66 | **`prepare_model_for_node()` can never transfer a model directory, so an admitted Soma container is never sent to a remote node.** `transfer_model_to_node()` handles directories properly — it walks them with `recursive_directory_iterator` and streams each file to `/api/node/models/receive`. But both of its callers sit behind `util::resolve_existing_local_model_path()`, which matches **`is_regular_file` only** and returns `nullopt` for any directory. So the directory branch of the transfer function is unreachable from placement, and a container falls through to "pass the ref along untouched" — the path an unadmitted model gets. On a single-machine deployment this is invisible, because control's absolute container path resolves on the node too. On a real multi-node cluster the node is handed a path that does not exist there, and the failure surfaces at load time as a missing model rather than as a transfer that never happened. Soma's models are ALWAYS container directories, so this is the normal case for the engine the branch exists to serve, not an edge one. Found while wiring D65, whose test had to use a single-file model precisely because a directory could not be charged for a transfer that cannot occur. | D65 implementation | **High — Soma is undeployable to a remote node through the normal placement path.** |
