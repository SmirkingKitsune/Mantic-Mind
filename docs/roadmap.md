...
**Open: D45 and D46, both low and both the same species — a description that outlived the thing it
described.** Everything else is closed except D4, which is parked with its reason recorded and its
error message fixed. All of D4–D8 came from a single run of the G8 gate against a real model — five
