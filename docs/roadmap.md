...
| D64 | **`PlacementFailure` is the one design element from D46's header with no successor: placement failure is still a bare string.** The deleted header proposed a seven-value structured code — `CapacityPressure`, `NoConnectedNode`, `ModelNotAdmitted`, `ModelTransferFailed`, `EngineUnavailable`, `Rejected` — with `last_failure()` beside `last_error()`. `grep -rn PlacementFailure src/ include/` is empty and `AgentScheduler::last_error()` returns a `std::string`, so every caller that wants to know WHY a placement failed has to match English prose — the exact defect the header called out one layer down, where `response_indicates_capacity_pressure()` fixed it for a single case by having both engines emit `{"error":{"code":"capacity_pressure"}}`. So the pattern is proven in this codebase and applied once. It matters most for the conformance gate added by the master-configurator work: "no node is conforming" and "every node is full" are opposite operator actions and currently differ only by wording. Not fixed with the header's deletion, because inventing an enum nothing consumes would repeat the mistake — it needs a caller that branches on it. | D46 resolution sweep | Medium — an operator or script cannot distinguish placement failure causes without parsing English. |
