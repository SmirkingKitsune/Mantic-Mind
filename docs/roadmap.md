...
| D62 | **Three-axis placement is three-axis on the supply side only; every demand figure is still VRAM.** `ResourceFootprint{vram, ram, disk}` landed properly — `NodeRegistry::nodes_with_capacity()` takes one, `evaluate_fit()` checks disk first and unconditionally, and `test_capacity_fit_across_three_axes` covers it. But every footprint control ever CONSTRUCTS sets `vram_mb` and leaves `ram_mb` and `disk_mb` zero: `agent_scheduler.cpp` builds one from `estimate_inference_vram_mb() + projector_file_mb()`, and `control_api_server.cpp`'s probe hardcodes a 2048 MB default. So a Soma-routed agent — whose real cost is RAM plus disk, and whose plan document states both — is placed against a llama.cpp-shaped VRAM estimate, and the disk axis can only ever reject on headroom, never on the model's actual demand. The three-axis footprint IS computed for Soma, but node-side in `engine_descriptor.cpp`, which is after placement has already chosen. Honestly commented where it happens, which is why this is a gap rather than a false claim. Found while resolving D46 — it is the one proposal in that header not yet fully delivered. | D46 resolution sweep | Medium — placement decisions for the streaming engine are made on the wrong axis. |
