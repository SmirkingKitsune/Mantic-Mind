...
| D46 | **`PlacementEngine` is a 175-line header that nothing implements, includes, or builds.** `include/control/placement_engine.hpp` declares the class with ~20 members, `BackendDecision`, `BackendReason` and `PlacementFailure`, and `grep -rn PlacementEngine --include=*.cpp` returns **zero hits**; there is no `placement_engine.cpp` and no CMakeLists names one. The live scheduler is still `AgentScheduler` — which the header describes in the past tense ("REPLACES AgentScheduler", "whose own doc comment read…"), so it reads as the current design rather than an unbuilt one. Same shape as the retired `ArchBackend`: a described contract with no implementation, where the description is what a reader trusts. It bit this work directly — `PlacementFailure::EngineUnavailable` is exactly the code the conformance gate wanted, and wiring against it would have compiled nothing. The gate went into `AgentScheduler::ensure_agent_running` instead. | master configurator | Low — dead code, but actively misleading about which class places agents; fix is to implement it or delete it, and either way stop describing it as the replacement. |
| D45 | **The planner documents a "configured KV dtype" that does not exist.** Both `kv_bytes_per_token()` implementations say the fp32 figure is what the G0 cache uses and that "the planner scales by the configured KV dtype". There is no such configuration anywhere: the cache is `std::vector<float>` and every plan reports fp32 bytes. Harmless today because nothing reads a knob that is absent — and misleading precisely where it matters, since KV competes with the expert cache for the same RAM and a reader may believe the reported figure is already scaled. Related but separate: `DType::F16` and `DType::BF16` are in the enum and in `quant.cpp`'s tables with NO kernel for either, and all three bind paths refuse a checkpoint that is not F32 — so bf16 weights, which is what essentially every model ships, cannot be loaded at all. | doc drift review | Low — a false comment and an unimplemented enum value; fixing it means either building the fp16 KV cache or deleting the claim. |
