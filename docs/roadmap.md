...
| D61 | **`soma/routing.hpp` said the routing decision is what `GET /v1/agents/{id}` answers. That route carries no backend field at all.** The header's opening paragraph justified the function's purity with "the same function answers `GET /v1/agents/{id}` — so a client can see the decision and its reason without causing a placement". The handler returns config, placement, status and node compatibility, and no `backend` or `backend_reason`. `GET /v1/placements` and `PUT /v1/agents/{id}/backend` do surface the reason string, so the capability exists — it is simply absent from the per-agent read, which is the natural place a client would look. The deleted `placement_engine.hpp` made the same promise, which is how the claim survived in two files at once. **The comment is corrected in this commit; the route is not**, because adding a field to an existing response is a behaviour change and belongs in its own commit. The fix is small and needs no new route: `resolve_backend_for(cfg)` is pure and causes no placement. | D46 resolution sweep | Low — a false claim in a header, now corrected, over a real three-line gap. |
| D60 | **`ControlModelRegistry::record_placement()` and the `placement_history` table are implemented and called by nothing.** The table is created on every control start (`CREATE TABLE IF NOT EXISTS placement_history`, plus an index on `(agent_id, placed_at DESC)`), the writer is declared and defined with backend, backend_reason and footprint_json columns, and `grep -rn record_placement --include=*.cpp` finds only the definition. So every deployment carries a schema and an index for a history nothing records, and `docs/architecture.md` lists "placement history" as a reason `control.db` exists. Exactly D46's shape one layer down — built, described as current, wired to nothing — found by the sweep that resolved D46. Fix is to call it from `ensure_agent_running` (the data it wants is all in hand at that point: agent, node, backend and reason) or to delete the table and the writer together. | D46 resolution sweep | Low — dead code plus a schema that implies a feature. |
