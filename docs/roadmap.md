...
| stage | result |
|---|---|
| convert | 499 GiB container, 108 shards, 0 padding, tokenizer compiled in |
| verify | 19,200 slots structurally in 0.167 s; 117 experts byte-exact across all 75 MoE layers |
...
its routed experts already stream through the memory hierarchy. The unused `ArchBackend` parallel
contract was never implemented or called and has been deleted rather than promoted into a second
forward path that would have to stay numerically aligned with this one.
...
##### Landed since the initial G3 design
...
The format tag is **backend-owned** and reached through `resolve_attention_backend`, so this core TU never
...
(`PlacementEngine` is the one item here that was never built: its whole contents shipped into
`AgentScheduler` instead, and the header was deleted — D46.)
...
**Open: D45 and D46**, both low and the same species — a description that outlived the thing it
described. Everything else is closed except D4, which is parked with its reason recorded and its
error message fixed.

D47 and D48 are worth reading together. They were found by one question — "does admission have a
TUI, and how does its parallelism behave" — and they had the same root: admission was only ever
driven one operation at a time, from a client nobody ships. Nothing was wrong with the code anyone
had looked at. The unbounded concurrency, the colliding container directory, the queued state that
existed and was never reachable, the terminal frame a cancelled-while-queued operation never
published — none of it was visible from the single-threaded path through an API-only surface. All of D4–D8 came from a single run of the G8 gate against a real model — five
defects that every unit test in the repo passed straight over, none reachable by any fixture, and the
two that blocked serving were each invisible until the one before it was fixed. That is the argument for
...
|---|---|---|---|
| D65 | **`disk_mb` is still zero for every placement, because nothing can say whether a node already holds the model.** The disk axis is enforced — `evaluate_fit()` checks it first and unconditionally — but only ever against the 8 GiB headroom, never against a model's actual demand, because every footprint control builds leaves `disk_mb` at 0. The reasoning is sound and unchanged since it was written: a container's residency is not ADDITIONAL to what the target already holds, and `NodeInfo` carries no local-model list, so charging the full container size would reject exactly the nodes that already have it — the cheapest ones to place on. It matters most for Soma, whose whole design streams a routed set from disk that can be hundreds of gigabytes. **The prerequisite is node-side residency reporting**: the node already answers `GET /api/node/models/local`, so the work is to carry it (or a per-model probe) into `NodeInfo` via the health poll, then charge `disk_mb` only when a transfer is actually needed. Split out of D62 rather than left inside it, because the VRAM half was a rejection bug with a fix and this half is a missing input. | D62 resolution | Medium — placement cannot refuse a node that lacks room for the container it is about to be sent. |
| ~~D63~~ | **`docs/external-api.md` published `GET /v1/placements`'s contract from the header that was never implemented.** The documented response was `{"placements": [{…}]}` with `engine_id`, `backend_detail`, `footprint`, `footprint_source` and `model_id` fields, over an enum `backend_reason ∈ verdict \| no_admission_record \| operator_override \| verdict_reject \| resident_only \| remote_api`. The route returns a **bare array**; each row is `AgentPlacement`'s nine fields (`agent_id`, `node_id`, `slot_id`, `suspended`, `is_active`, `kv_cache_node_path`, `engine_fingerprint`, `placed_at_ms`, `last_active_ms`) plus `backend` and `backend_reason` added by the handler. None of `engine_id`, `backend_detail`, `footprint`, `footprint_source` or `model_id` is emitted. And `backend_reason` is not an enum token but a composed sentence from `BackendDecision::explain()` — `soma (verdict=hybrid)`, `llama-cpp (override_refused_conformance, verdict=reject)` — whose reason component comes from the LIVE `soma::BackendReason`, in which `verdict_reject`, `resident_only` and `remote_api` do not exist and `stale_admission_record` and `override_refused_conformance` do. Every wrong value traces to `placement_engine.hpp`'s `BackendReason`: the doc was written from the design header, not from the code. **This is the sharpest form of the D46 defect** — a stale comment misleads a maintainer, a published API contract misleads a client, who codes against it and gets fields that never arrive. `check_api_docs.py` did not catch it because it verifies that a documented route is REGISTERED, not that its documented body matches what the handler emits. Corrected against the source. | D46 resolution sweep | Resolved |
| ~~D64~~ | **Placement failure was a bare string, so `no node is conforming` and `every node is full` — opposite operator actions — differed only by wording.** A deleted design header (D46) proposed a structured code and was compiled by nothing; the live scheduler kept `last_error()` as a `std::string`, and every consumer rendered it. **Resolved with a taxonomy read OUT of the code rather than copied from the dead header** — that header's `Rejected`/`EngineUnavailable` matched nothing the scheduler does, and importing them would have shipped codes no caller could observe, which is the D63 mistake. Each of the eight values corresponds to a real `set_failure()` site: `engine_config_missing`, `no_local_backend`, `no_eligible_node`, `no_capacity`, `model_transfer_failed`, `node_rejected`, `node_unreachable`, `node_protocol_error`. **The motivating pair is now genuinely distinguished**: the fall-through asks `registry_.available_nodes().empty()` — the connected + conforming filter — so nothing eligible reports `no_eligible_node` and eligible-but-full reports `no_capacity`, where both previously emitted the same sentence. **Three real consumers, because an enum nothing branches on would have repeated the mistake this closes.** Internally, the fall-through's `if (last_error().empty())` — control flow that depended on PROSE, the same defect one layer down — became `last_failure() == None`. On the wire, the two placing routes emit `code` and `retryable` beside the message. `retryable` is biased toward true where arguable: a wrong true costs one wasted poll, a wrong false abandons a placement that would have worked, so only the two that genuinely need a human (`engine_config_missing`, `no_local_backend`) are false. The existing engine-config gate test asserted the distinction by STRING MATCH; it now asserts the codes, so rewording a message can no longer keep a passing test while breaking every client. A second test pins eligible-but-full against a live node reporting 1 MB of VRAM, **verified by mutation** — collapsing the branch fails it. | D46 resolution sweep | Resolved |
| ~~D62~~ | **Placement charged a CPU-only engine for VRAM, and a GPU-less node was therefore refused outright.** `ResourceFootprint{vram, ram, disk}` reached `nodes_with_capacity()` properly, but every footprint control CONSTRUCTED set `vram_mb` alone, from llama.cpp's `estimate_inference_vram_mb()`, for both engines. Soma v1 is CPU-only — `plan.cpp` sets `vram_hot_bytes` and `footprint.vram_bytes` to zero and says so — yet a Soma agent arrived at `evaluate_fit()` asking for VRAM, and that function refuses to offload against a host below `min_gpu_for_offload_mb` (8 GiB). **So the failure was a rejection, not a bad ranking**: a GPU-less node could not host a Soma agent at all, however much RAM it had, for VRAM the engine would never touch. Logged as "places on the wrong axis"; it was worse than that. **Fixed** with `soma_footprint()`, chosen by the resolved engine: `vram_mb` stays 0, so `evaluate_fit()` short-circuits to Native and RAM and disk are still checked; `ram_mb` is the RESIDENT half — model bytes minus `total_routed_bytes` — which is the part that must be held whatever expert cache the host picks, and therefore the part control can state without knowing the target's budget, which is precisely what `plan_for_host()` refuses to guess. It is a deliberate LOWER BOUND, excluding KV and the expert cache: under-charging risks a node re-deriving a tighter plan at load, over-charging invents the node's cache policy on control and rejects hosts that would have worked. Tested by contrast — the same CPU-only host accepts the RAM-shaped footprint as Native and refuses the VRAM-shaped one with "no GPU large enough", and an oversized resident half is still refused, so this is not "Soma fits everywhere". **The disk axis is NOT closed and is carried forward as D65** rather than claimed here. | D46 resolution sweep | Resolved |
| ~~D61~~ | **`soma/routing.hpp` said the routing decision is what `GET /v1/agents/{id}` answers. That route carried no backend field at all.** The header justified the function's purity with "the same function answers `GET /v1/agents/{id}` — so a client can see the decision and its reason without causing a placement". The handler returned config, placement, status and node compatibility, and no `backend` or `backend_reason`. `GET /v1/placements` and `PUT /v1/agents/{id}/backend` did surface the reason, so the capability existed and was simply missing from the per-agent read — the natural place a client would look. The deleted `placement_engine.hpp` made the same promise, which is how one false claim came to live in two files. **Fixed rather than re-worded**: the route now calls `resolve_backend_for(cfg)` and emits both fields. Purity is what made that safe — a read route must not cause a placement, and this one still does not. | D46 resolution sweep | Resolved |
| ~~D60~~ | **`ControlModelRegistry::record_placement()` and the `placement_history` table were implemented and called by nothing — and had no reader either.** The table and an index on `(agent_id, placed_at DESC)` were created on every control start, the writer was defined with backend, backend_reason and footprint_json columns, and `grep -rn record_placement --include=*.cpp` found only the definition. `docs/architecture.md` listed "placement history" as a reason `control.db` exists. **Wiring only the writer would have made it worse**: an unbounded write-only table is a capability with no way to reach it, which is precisely what P1 forbids — so both ends landed together. `AgentScheduler` gained a `PlacementAudit` callback pair (a pair of callbacks rather than a second registry pointer, because the scheduler holds the registry as `const*` on purpose: it reads verdicts and must not be able to write model rows), `mark_placement_released()` closes rows, and `placement_history()` reads them back onto `GET /v1/agents/{id}` — an EXISTING route, so no new scope entry, CLI verb or TUI control was needed and both parity directions stayed at zero. **Two details that are the whole correctness of it.** Audit events are queued and flushed by a guard destroyed AFTER the scheduling lock releases: `schedule_mutex_` already serializes multi-GB transfers and has no business also holding a synchronous SQLite insert, and a callback invoked under a lock is the shape that killed the node in D56. And the release stamps **the most recent OPEN row only** — an agent placed, released and placed again has two rows, and closing "the row for this agent" would clobber the first placement's duration; verified by mutation, which fails exactly that assertion. The audit hangs off the two publish sites rather than off `store_placement()`, because that is also called to refresh an unchanged placement and would have recorded a row on every `ensure_agent_running`. | D46 resolution sweep | Resolved |
| ~~D59~~ | **The engine-config route's own comment said the push does not block on provisioning; the second push blocked on it for the whole build.** `POST /api/node/engine-config` answers immediately and hands off to `apply_async`, with the comment "provisioning runs in the background because a source build takes minutes and control's push must not block on it". True for the first push. `apply_async` joined the *previous* worker before starting the next — **on the caller's thread**, which is the HTTP handler — so a config change arriving while a build was running waited out the build, and control's 30 s health poll marked the node unreachable in the meantime. Latent until now for an ugly reason: the node died on the first push (D56), so it never survived long enough to have a build in flight when a second arrived. Fixing D56 is what made this reachable. **Fixed** by moving the join inside the new worker — the "no two concurrent applications" property is preserved (they still serialise, and a half-installed runtime that neither call owns is still impossible), the caller no longer waits. | node crash investigation | Low — latent until D56 was fixed, then a hang under repeated config changes. |
| ~~D58~~ | **The node could not find the engine that ships with it.** `SomaEngineProvisioner::ensure()` resolved `soma` through `resolve_llama_executable`, which searches PATH (and, on Windows, the app directory). The documented rule — in that function's own error message — is that soma "is built with the node" and ships beside it. In a build tree those are different directories: soma lands in `src/soma/`, the node in `src/node/`, and nothing ever put a copy next to the other. So on every fresh build, a cluster config naming Soma as primary reported `absent` for a binary sitting one directory away, and the operator's log line said to check `MM_SOMA_PATH` without naming a single place it had looked. Control had the same fault for a different reason: admission shells out to `soma plan` and `soma conform`. **Fixed on both sides of the claim.** The build now copies `soma` (and its runtime DLLs) beside `mantic-mind` and `mantic-mind-control`, `install(TARGETS soma)` puts it in the same `bin` after `cmake --install`, and resolution consults the running executable's directory — via a new `util::executable_dir()`, which asks the OS rather than trusting `argv[0]` or the working directory — **before** PATH. That order is deliberate: a `soma` that happens to be on PATH belongs to some other install, and Soma is versioned with the node. Copying rather than teaching the node about `../soma/Release/`, which is build-tree structure that does not exist in a deployment. The failure message now names both places it looked. | node crash investigation | Medium — Soma, the primary engine, was unusable on every fresh build. |
| ~~D57~~ | **The crash was invisible in the file written to explain it.** The report was "the node closes without notice and does not even show in the logs", and the second half was its own defect. `init_logger` sets `flush_on(warn)` — added by D20 so a dying process's last *warning* reaches disk. A process that `abort()`s does not warn on the way out, so every info line since the last warning stayed in the buffer and was lost. In the reported crash the log ended four lines early, at an unrelated discovery warning, omitting the line naming the config the node had just been handed — the one fact that identified the trigger. **Fixed** with `spdlog::flush_every(2s)` behind the existing `flush_on(warn)`: a periodic flush on one shared thread, which costs a wakeup and buys "the last thing the process did is on disk". Two seconds rather than sub-second because the consumer is a human reading after the fact. | node crash investigation | Medium — not a fault by itself, but it hid one and would hide the next. |
| ~~D56~~ | **A lock held across a callback killed the node on every cluster config that named llama.cpp.** Reported as "the node closes without notice"; the Windows fault record says `0xc0000409` parameter 7 — `FAST_FAIL_FATAL_APP_EXIT` from `ucrtbase!abort`, i.e. `std::terminate` on an uncaught exception, and MSVC's default terminate handler prints nothing. **The exception was `resource deadlock would occur`**: MSVC's `std::mutex` refusing a recursive lock by throwing rather than hanging. `LlamaEngineProvisioner` used one mutex for both jobs and held it across `ensure_runtime()`; that call reports progress by invoking the progress sink; the node's sink asks the engine manager for llama status, which comes straight back into the same object and takes the same mutex on the same thread. It also had a genuine two-thread inversion behind it — accessors run manager→state, the sink ran state→manager — so a recursive mutex would have converted a reliable crash into an occasional hang. **Fixed** by splitting into `op_mutex_` (serialises long operations, held across them, never taken by an accessor) and `state_mutex_` (guards the fields, never held across a call into `LlamaCppProvisioner`), with the provisioner handle as a `shared_ptr` so the unlocked call is provably safe rather than safe-by-argument. All six operations had the bug, not just `ensure` — update, switch, diagnose and recover emit progress too, so the TUI's wizard buttons were the same crash by another route. **And a boundary behind the fix**: `NodeEngineManager::apply()` now catches per engine (per engine, so a llama.cpp that throws cannot stop Soma resolving behind it — the optional-backup claim, which one shared catch would have quietly retracted), the worker thread has a catch-all, and a provisioner that throws is reported as `error` with the reason rather than inheriting `absent`, which means *nothing tried*. Three tests: the reentrancy, driven through the real wrapper with an injected `LlamaCommandRunner` so it reaches the progress callback without a download, and **verified by mutation** — pointing one accessor at the operation lock reproduces the throw at the first progress frame, exactly where production died; the exception boundary, via an injected throwing provisioner; and the sibling resolution (D58). | node crash investigation | **High — the node died within one health poll of every config push naming llama-cpp, primary or backup, leaving no log line and no exit.** |
| ~~D55~~ | **The last nine API capabilities with no screen — closed, and both parity directions are now enforcing.** Remaining after D54: per-sequence engine slots, the model plan document, conversation-local memories (4 verbs) and the curation propose/apply loop (3). All nine now have controls. Slots are parsed from the node's real `{sequences:[…]}` document and rendered with agent, phase, position, KV occupancy and determinism; a route literal hidden in a poller is not a screen. The plan view likewise renders the endpoint's real admission-host evidence rather than inventing current-host RAM/disk projections the route does not return. Two notes on HOW, because both were choices rather than mechanics. The curation and local-memory controls go over **HTTP rather than `a->db()`** like the rest of that tab: the db() path bypasses ConversationManager and MemoryManager as well as the API handler, and the propose/apply loop has real logic in the handler that a direct table write would skip — so routing them through the API both avoids growing the in-process mutation surface and moves the TUI toward what P1 says it is. Apply retains and submits the exact proposal documents returned by propose, including their before/after snapshots; an empty body is rejected by both apply aliases and therefore is not a control. And conversation-local memories got their **own id and display vectors**: `cur_mem_entries` holds the agent-WIDE memories, and indexing one list to edit the other addresses the wrong row silently, since the two scopes render identically. The local list is cleared when agent or conversation selection changes, so an id from one scope cannot be sent to another. **TUI coverage is now fatal**, matching the CLI half — a check stays advisory exactly as long as a pre-existing backlog makes failing it unfair, and no longer. Final state: CLI 72 reachable / 8 exempt / 0 gaps, TUI 69 reachable / 11 exempt / 0 gaps, 15 TUI mutations mapped, across 80 routes. **A third matcher limit found by mutation and recorded rather than fixed**: segments are matched INDEPENDENTLY, so sibling routes cover for each other — breaking `/curation/proposals` was not caught because `/curation/proposals/apply` still supplied both segments. Removing the only user of a segment IS caught (breaking `/local-memories` failed all four of its routes). That is a real bound on what the numbers mean, so it is in the header's "what this cannot do" list beside the other two, which is now kept current as each limit is discovered rather than written once. | TUI parity audit | Resolved |
| ~~D54~~ | **`check_surface_parity` watched three references and called that "every in-process mutation" — the Curation tab mutates through a fourth.** Check 2 asserts the TUI introduces no capability the API lacks, by scanning `control_ui.cpp` for calls on `agents_`, `registry_` and `scheduler_`. The Curation tab uses none of them: it takes an agent handle and reaches `a->db()`, the raw `AgentDB`. So `set_active_conversation`, `create_conversation`, `delete_conversation` and `delete_memory` were TUI mutations the check never saw — the same shape of mistake as a gate registered for one architecture (D49), one layer up. Worse than a coverage miss: those calls bypass `ConversationManager` and `MemoryManager` as well as the API handler, so the drift risk is two layers deep, not one. **Fixed** by watching `db().*` too and mapping all four to their routes; mutations mapped went 9 → 15. **The TUI coverage direction did not exist at all** and now does: 60 routes reachable, 11 exempt with recorded reasons, 9 remaining gaps, reported per route rather than as a number. Two high-value ones closed in the same pass — Suspend/Restore buttons on the Agents tab (the placement verbs reached the API in D51 and the CLI with it, while the one screen showing which agents run had no control, so freeing a node meant dropping to a shell) and Reprofile on the Admissions tab. **Two matcher faults found by running it, not reading it**: `POST /v1/nodes/:id/forget` reported as a gap because the mutation map already proved it and the checker re-derived it from literals anyway, wanting a lowercase "forget" the button spells "Forget Pairing" — a check that doubts its own evidence produces noise, and noise is what gets a check switched off; mapped routes now count directly. And adding the two buttons made the check FAIL until they were mapped, which is the property working. Remaining gaps are declared, not exempted: engine slot detail, the model plan document, conversation-local memories, and the curation propose/apply loop. | TUI parity audit | Resolved |
| ~~D53~~ | **Sixteen `/v1` routes had no CLI form, and nothing counted them.** D51 closed the P1 *violation* — a capability the API could not reach at all — but left the coverage direction unmeasured: routes the API exposes that no CLI client can drive. A mechanical diff found 16, and the sharpest were the three **token** routes: `GET/POST /v1/tokens` and `DELETE /v1/tokens/:id` had no CLI form, so a headless deployment could not mint its FIRST credential. Bootstrapping required either the legacy flat `external_api_token` from config or hand-rolled HTTP — the scoped-auth system was, in practice, unreachable on exactly the deployments it was designed for. Also missing: the registry's whole administrative half (`models delete|register|reprofile|verdict|show|plan|conformance|heat`), `performance` read and reset, the live-engine readers (`engines running|heat|slots`), conversation-local memories (all four verbs), curation propose/apply, the six voice-proposal routes, and `PUT /v1/agents/:id/backend` — the one place an operator can overrule a verdict, which was a TUI-only act. **Fixed**: 30 CLI route calls now, up from 21; coverage is **72 reachable, 8 exempt with a recorded reason, 0 gaps**. The exemptions are all shape mismatches rather than omissions — an unbounded SSE stream, binary audio, multipart image upload, and the OpenAI-compat spelling of a capability `chat send` already covers. **`check_surface_parity` now enforces the coverage direction**, and became fatal only once the backlog reached zero: failing the build for gaps that predate a check teaches people to disable it. **Three bugs in the checker itself, each found by mutation rather than by reading it.** It saw only paths written inline at the call site, so `std::string path = "/v1/activity?tail=" + n; self.get(path)` was invisible — and an invisible call cannot fail check 1 either. It collected only `/v1…` literals, so concatenated fragments like `"/conversations/"` were missed and `curation conv activate` reported as a gap. Worst, it matched segments by SUBSTRING: with the `agents backend` command deleted outright it still reported the route covered, because "backend" occurs inside the unrelated literal "backend_override". A checker that reports coverage which does not exist is worse than no checker — it converts an unknown into a false assurance. Whole-token matching now, verified by deleting the command and watching it fail. | P1 coverage audit | Resolved |
| ~~D52~~ | **Conformance stage 3 — the only DISTRIBUTIONAL correctness gate in the ladder — was built and never registered, so it ran for no architecture at all.** `soma_stage3_g2` is an `add_executable` with no CTest entry, on the stated grounds that it "needs a real container and a real reference, and neither belongs in the repo. CI runs stages 1-2 on the committed tiny fixtures." That was true when written. Both halves stopped being true since: every tiny fixture carries `tf_logits` — a real teacher-forced transformers reference, committed, generated by `make_oracle.py` — and every family now has a converted container (D50 supplied the last one). Meanwhile stages 1-2 assert *token-exactness on tiny-random weights*, which is a different property from "the engine's logits match what transformers produced", so nothing in CI was checking the latter for any family. **Verified before registering** rather than assumed: all six families pass at 0.1 s each, mean KL 0.00000–0.00029 nats against a 0.050 bar (GLM-5.2: mean 0.00029, p95 0.00118, top-1 agreement 91.2%). And it **discriminates** — pointing a container at another family's reference fails, which is what makes registering it worth anything; a gate that cannot fail is worse than no gate. Registered as `soma_stage3_{gqa,mla,mla_sigmoid,coarse,mha,dsa}_g2`; suite 45 → 51. **`mm_test_registry` caught two attempts at getting this wrong**: a `foreach()` loop registers tests whose names it cannot grep statically, and then a comment quoting its own grep pattern, which it read as a test literally named "...". Both are the D44 guard doing exactly its job. | GLM stage-3 verification | Resolved |
| ~~D51~~ | **Suspend, restore and release were internal-only capabilities — the exact thing P1 forbids.** `AgentScheduler::suspend_agent()` was **private**; the scheduler calls it itself under capacity pressure, and the node API has always exposed `/api/node/suspend-slot` and `/api/node/restore-slot`. On the control side `GET /v1/placements` was the only placement route in the table — read-only. So an operator holding the entire control API could not suspend, restore or release an agent, while the scheduler did all three on its own. P1 states it in the API document's own words: "there are no TUI-only features and **no internal-only capabilities**". The gap was *known and written down*: `include/control/placement_engine.hpp` says "Both exist today but only as internal scheduler decisions reachable via the node API — there is no /v1/* route for either. Promoted here because P1 says a capability the system has is a capability the API exposes." That header is compiled by nothing (D46), so the note never became a route and nothing ever failed. **Fixed**: `suspend_agent` promoted to public, plus `POST /v1/agents/:id/{suspend,restore,release}` (all `operator`), CLI `agents suspend|restore|release`, and `placements` as a CLI reader for the route that had none. Restore is `ensure_agent_running` rather than a second path — its first step is already "existing/suspended placement", so a suspended agent restores from checkpoint instead of reloading, and a parallel implementation would be a second copy of the placement ladder. Suspend answers **409** (not 404) when the agent exists but holds no placement: an operator script needs to tell those apart. **And guarded**, because fixing the instance does not stop the recurrence: `mm_surface_parity` asserts every CLI `/v1` call lands on a registered route and every in-process mutation in `control_ui.cpp` has a `/v1` equivalent in the scope table. Verified by deleting the release route and watching it name exactly that mutation. The check states plainly what it cannot do — it cannot prove a button and a route behave the same, and the curated map of TUI mutations is the honest residue, listed rather than hidden so it shrinks on purpose. | P1 parity audit | Resolved |
| ~~D50~~ | **GLM-5.2 was the only architecture with no container fixture, so every container-dependent gate was structurally blind to the only DSA family.** `tests/fixtures/tiny/` and `tests/fixtures/tokenizers/` both carry GLM-5.2; `tests/fixtures/containers/` carried five families and not it. Any gate that opens a `soma.container` therefore could not run against MLA+DSA at all — `soma_streaming_g2`, `soma_union_g3` and `soma_streamed_determinism_g3` each exit 2 with "container open failed: no soma.container" when pointed at GLM-5.2. Nothing reported this, because none of those gates was ever *registered* for GLM-5.2; the absence was invisible from a green run. GLM-5.2 is also missing from the fixtures README's family table, which is how it stayed unnoticed. **Fixed** by converting the committed tiny checkpoint with the current `tools/admission/convert.py` (q4_g / q6_g / g128, 2.8 MB) and committing `tests/fixtures/containers/GLM-5.2`. All three gates then pass against it and are registered as `soma_streaming_dsa_g2`, `soma_union_dsa_g3`, `soma_streamed_determinism_dsa_g3`. The container carries `tokenizer: unsupported` because the tiny checkpoint dir holds no tokenizer files — consistent with the other five, which carry no compiled tokenizer either; the GLM tokenizer is covered separately by `soma_tokenizer_g0`. | P1/GLM coverage audit | Resolved |
| ~~D49~~ | **Five Soma gates took a fixture-name argument and were only ever registered for Qwen3, so every assertion in them was made against one attention family — and one of them was asserting a GQA-only assumption.** `streaming_g2`, `union_g3`, `threading_g3`, `streamed_determinism_g3` and `autotune_g1` all accept an optional `argv[2]` model name defaulting to `Qwen3-30B-A3B`; only `scheduler_g3` and `checkpoint_g3` had MLA and DSA registrations. Pointing `streaming_g2` at DeepSeek-V2-Lite produced **six failures on a correct engine**: it hardcoded layer 0 for every acquire, churn, LRU and prefetch operation, which holds for Qwen3 (no dense prefix) and is false for DeepSeek (`first_k_dense_replace=1`) and GLM-5.2 (`=3`). A dense layer owns no routed experts, so "acquire returns a live reference", "the churn actually caused eviction", "LRU victim was the least-recently-used" and "prefetch on an enabled layer fetches" were all reading a layer that has nothing to read. The engine was right; the test's assumption was the bug, and it was unfalsifiable while one family ran it. **Fixed** by deriving the routed layers from `ArchIr::is_moe_layer()` once at the top and driving every section from the first two — `streaming_g2` now passes on all five container fixtures (routed layers 4/4 for Qwen3, Mixtral, OLMoE; 3/4 first=1 for DeepSeek and Moonlight). **Eleven per-family registrations added** across the five gates, verified individually before registering rather than assumed: the suite went from 33 to 44 tests. | P1/GLM coverage audit | Resolved |
| ~~D48~~ | **Admission concurrency is unbounded, and two concurrent admissions of the same model write the same directory.** `ControlModelRegistry::start_operation` spawns a **detached thread per call** — no queue, no semaphore, no cap; `grep -niE "max_concurrent\|semaphore\|throttle\|in_flight"` over model_registry.cpp finds nothing. N simultaneous `POST /v1/models/admit` calls therefore run N conversions at once, each spawning Python subprocesses and moving tens to hundreds of GB. **The sharp edge is a collision, not the load.** The container path is deterministic — `containers/<name>-<quant>-<expert_down>-g<group>` (model_registry.cpp:1184) — and the fetch destination is `sources/<name>` (:1199). Nothing dedups an in-flight source, so admitting the same model at the same quantization twice concurrently gives two `convert.py` processes interleaving writes into one directory, ending in a corrupt container with a registry row that describes it as good. The per-op `tools_copy` at :1091 shows concurrent admissions WERE anticipated — its comment reasons carefully about two quantizations of one model in flight together — but only the argument-shadowing half was handled; the same-quant case lands on the same path with no guard. **Never exercised**: both admission tests drove exactly one operation, so nothing in the suite had ever had two in flight. **Fixed**, in three parts. (1) The duplicate JOINS rather than starting a second — `start_operation` looks up a live operation by the container variant and, finding one, attaches the caller's sink to it and returns its id, so the second caller of a double-clicked button gets the progress stream they asked for instead of a corrupt container. Refusing would also have been defensible; starting a second was not. (2) The collision key is now `admission_variant()`, the SAME pure function that picks the write path — `run_admission` derived it inline, and adding a second derivation for the check would have been this defect again one layer up, a guard watching a different directory from the one being written. (3) Concurrency is capped, default 1, configurable via `admission_max_concurrent` / `MM_ADMISSION_MAX_CONCURRENT`; operations past the cap wait on a condition variable and report the `queued` state the protocol already had and never used. The wait is in the WORKER, not in `start_operation`, so a queued admission returns an id immediately instead of hanging the HTTP thread. **Two adjacent faults surfaced while fixing it**: a cap of 0 would have parked every admission on a gate nothing could open (clamped to 1), and an operation cancelled while still queued published no terminal frame at all — `run_admission` never ran — leaving a watcher waiting forever for the `done` the API guarantees on every path. Verified by `admission_variant_is_the_collision_key` (pure: a repo id and a directory for one model produce one variant; two quantizations produce two) and `concurrent_admission_of_one_model_joins_not_duplicates`, which drives two real admissions and asserts one id; stubbing the lookup to `nullptr` makes it fail on `id_a == id_b`. | admission review | Resolved |
| ~~D47~~ | **Admission has no operator surface at all — it is API-only.** `grep -rn "admit\|admission"` over `control_ui.cpp`, `soma_panels.cpp` and `soma_dashboard.cpp` returns **zero**, and the CLI REPL never calls `/v1/models/admit` (its `models` group is `list` alone). So the one operation in the system that runs for HOURS, consumes hundreds of GB, and has a full progress/cancel/rejoin protocol — SSE stages `fetch → convert → tokenize → profile → conformance → finalize`, `POST /v1/models/admissions/:op/cancel`, and `GET /v1/models/admissions/:op` explicitly built so a disconnected client can rejoin — is reachable only by hand-rolling HTTP. The Soma tab renders engine heat and memory tiers beside it. P1 says the TUI is one client of the API and there are no TUI-only features; this is the mirror failure nobody stated — an API capability with no client, where the progress protocol built for an operator has no operator to show it to. **Fixed** with an Admissions tab (control TUI, key `0`) and a `models admit|admissions|cancel` CLI group, both clients of the same routes under the same discipline as the Soma and Engines tabs — own translation unit, everything over `/v1/*`, guarded by `tools/ci/check_ui_api.py` (now 8 files). **Rejoin came free**: the tab POLLS `GET /v1/models/admissions`, which lists operations whether or not anyone is streaming, so closing the TUI mid-conversion and reopening it shows the admission still running with no reattach protocol on the client side. The SSE stream is used for exactly one thing — learning the operation id when starting one, because `POST /v1/models/admit` answers only as a stream — and then dropped, which is safe by design: control logs `client disconnected; conversion continues` and the worker is detached precisely so it outlives the request. Starting one runs off the UI thread, because a TUI that freezes while starting a job is how an operator concludes it hung and starts a second. **The renderer test found a bug in itself**: `mm_admission_panels` draws the panels to an off-screen buffer and asserts them as text, and the first version of the stage ladder indexed one flat list from `fetch` — which is wrong for every local-source admission, where there is no fetch stage, so every label shifted by one and `finalize` was never drawn. The ladder is now keyed on `total_steps` (3 = container, 7 = local, 8 = fetched) and an unrecognized length draws positions rather than inventing names: position comes off the wire and stays right, names are the part a client can be wrong about. Nine assertions cover the states nobody opens the app in — unreachable vs. idle (which render identically if you are careless), queued, failed, canceled, out-of-range selection, and both non-default ladder shapes. | admission review | Resolved |
| ~~D46~~ | **`PlacementEngine` was a 175-line header that nothing implemented, included, or built — and every idea in it had already shipped into `AgentScheduler`.** `grep -rn PlacementEngine --include=*.cpp` returned zero hits for the header's whole life; there was no `placement_engine.cpp` and no CMakeLists named one. It described itself in the past tense ("REPLACES AgentScheduler"), so it read as the current design rather than an unbuilt one — the same shape as the retired `ArchBackend`. **Resolved by deleting it**, after checking each of its claims against the live scheduler rather than assuming either way. Most had already shipped: backend selection recorded with a reason is `AgentScheduler::BackendRouting{engine_id, reason}` via `resolve_backend()`/`resolve_backend_for()` delegating to `soma::select_backend()` and keeping its `explain()`; `ResourceFootprint{vram, ram, disk}` exists in `common/footprint.hpp` and reaches `NodeRegistry::nodes_with_capacity()`, with disk headroom checked on every placement; capacity pressure is a structured `{"error":{"code":"capacity_pressure"}}` from both engines, the six-phrase substring match surviving only as a rolling-upgrade fallback for a node too old to emit a code; the two-mutex split it promised to keep was never removed; and suspend/restore reached `/v1` in D51. Its own motivating quote had also gone stale — the `is_llama_backend` gate it cited as "the first thing a Soma agent hits" now separates node-local from API-backed agents *inside* `resolve_backend()`, and Soma passes straight through it. **Two did not ship, and saying "every idea in it shipped" would have replaced one false claim with another.** `PlacementFailure` — a seven-value structured failure code — has no successor at all: `grep -rn PlacementFailure src/ include/` is empty and `AgentScheduler::last_error()` is still a bare string, so a client still cannot tell capacity pressure from a model-transfer failure without reading English (logged as D64). And the footprint's demand side is still VRAM-only (D62). Both are carried forward as open rows rather than buried in the deletion. Implementing the header would still have meant a second scheduler competing with a working one — and it could not even have been revived incrementally: the header redeclared `mm::ScheduleResult` with a third member (`engine_id`) against the live two-member definition in `agent_scheduler.hpp`, so including both in one translation unit is a redefinition error. A dead header that cannot be included is not a design waiting to be adopted. Four documents described it as real and were corrected: `docs/architecture.md` §8's before/after table (with a new §8.0 mapping each proposal to where it actually lives), `docs/mantic-mind-integration.md` §2, `docs/external-api.md` §4, and three code comments that cited the file as evidence. **The lesson is kept rather than the header**: the suspend/restore gap this header correctly identified stayed open precisely *because* it was identified somewhere no build could check — being right in an uncompiled file fails nothing. | master configurator | Resolved |
| ~~D45~~ | **The planner documented a "configured KV dtype" that does not exist — and the numbers written beside it were half the real ones.** Both `kv_bytes_per_token()` implementations said the fp32 figure is what the G0 cache uses and that "the planner scales by the configured KV dtype". There is no such configuration: `KvCache` holds `std::vector<float>`, `plan.cpp` multiplies the figure by context and `kv_slots` and by nothing else, and no flag, env var or container field selects a KV width. **Worse than the phantom knob, and not in the original report: three comments quoted fp16 arithmetic beside implementations that multiply by `sizeof(float)`.** `attention_backend.hpp` worked Qwen3-30B-A3B at 98 KB/tok and DeepSeek-V2-Lite at 31 KB/tok — both fp16 — and `plan.cpp` said Qwen3's cache "wants 3.2 GB" at 32k context. The real fp32 figures are 192 KiB/tok, 61 KiB/tok and **6.4 GB per slot**: exactly double, on the optimistic side, for the one quantity those comments exist to say is *not* a footnote. **The arithmetic was always right** — nothing ever mis-planned — which is the danger: a reader checking the code against its own comment would have "fixed" a correct implementation. Resolved by deleting the phantom knob and correcting the figures, not by building an fp16 cache. **The second half of this defect as originally logged was wrong, and is corrected here rather than quietly dropped.** It claimed `DType::F16`/`BF16` are unimplemented enum values and that "bf16 weights, which is what essentially every model ships, cannot be loaded at all". They are not decorative: `parse_safetensors_dtype()` needs them to name a source tensor's dtype, and `dtype_bits()` is what validates shard byte lengths at `safetensors.cpp:294` — remove them and no HF checkpoint can be read. Nor are bf16 models unloadable: `tools/admission/convert.py` reads them directly ("Real checkpoints are bf16 … bf16 is the top 16 bits of an f32, so nothing is lost") and admission is the *only* intended route to serving. What is true is narrower: there is no kernel to bind a raw f16/bf16 tensor, and `serve` already refuses `--quant-dense f16` immediately with "is not implemented by the resident-weight loader" — an honest refusal, not a silent failure. | doc drift review | Resolved |
| ~~D44~~ | **The suite was reporting 29 tests when 31 were registered, and the two missing ones were the two that mattered most.** `ctest` reads the build directory's generated CTestTestfile, not CMakeLists.txt, and it had gone stale: `soma_checkpoint_mla_g3` and `soma_checkpoint_dsa_g3` — the MLA and DSA KV-checkpoint round-trips — were defined and never run. Every "29/29 passed" in the dead-V-plane work was therefore a green light from a suite missing the exact cases that change touched, since MLA is the family whose V plane vanished and DSA the one whose plane narrowed. **Fixed** by re-configuring (`cmake .`); all 31 pass, including both round-trips, so the change is verified against the cases it needed — the verification was wrong, not the code. **And guarded**, because re-configuring fixes the instance and not the recurrence: `mm_test_registry` compares every `add_test(NAME ...)` in the tree against what the generated CTestTestfiles actually register, and fails naming each missing test and the cmake command that fixes it. It can check itself because staleness only ever HIDES NEW tests — a stale file is an older complete one — so once registered it keeps running and keeps seeing what was added after it. Verified by deleting the two checkpoint entries from the generated list and watching it name exactly those two. **Two process faults compounded it.** Running a test binary explicitly gave a spurious FAIL from a STALE executable, because build output had been filtered with `grep -E "error C"`, which matches compiler diagnostics and not linker ones — a partial build failure passed unnoticed and left old binaries behind. And `ctest -N | grep -c` was read as confirmation that only one checkpoint test existed, when it was reporting the same staleness. Build success is now taken from the exit status rather than from a pattern chosen to match the errors that were expected. | dead V plane | Resolved |
| ~~D43~~ | **`soma_engine_g5`'s batching assertion depended on four requests coinciding, which the test could not force.** Reproduced at 1 run in 25, and the evidence I had first recorded was wrong: the `EngineSupervisor: engine <uuid> crashed` line I cited is EXPECTED output from a deliberate start-up-failure test, and occupying port 8123 does not reproduce it at all. The real failure is `the engine really did batch them   FAIL   max observed batch = 1`. **Three causes, separated by measurement.** (1) The check sampled `current_batch` — an instant — over HTTP, so a batch that formed and drained between samples left no trace; a `max_batch_seen` high-water mark on the scheduler now answers the question with a fact, and runs where the sampler saw 1 while the engine reached 3 or 4 confirm the race was real. (2) The four callers were spawned in a loop, staggering their starts enough that the first could finish before the last arrived; they now release from a shared barrier. (3) Even simultaneous requests sometimes fail to overlap, which disproves nothing about an engine that CAN batch — so the observation is retried up to three times while the correctness checks (identical output solo vs batched, distinct answers, no splicing) still run on every attempt and any failure remains recorded. The gate was measured at 4 in every run including failing ones, so admission was never the constraint. 25/25 after, from 24/25 and 19/20 at the intermediate steps. | indexer scoring | Resolved |
| ~~D1~~ | **Not a transport flake — a PORT COLLISION that served the test another process's server.** `find_free_test_port()` binds, closes, and returns the number, so the caller binds it a moment later; within one process a monotonic counter made that harmless, but EVERY process started at 42800 and marched upward in lockstep. When two raced, the loser's `listen()` failed, the winner's server satisfied the readiness poll — which accepts any responder — and the test asserted against a STRANGER returning plausible wrong statuses. That is why it read as transport-layer and why adding transport retries did not cure it: retrying against the wrong server does not help. **Measured**: 60/60 in isolation, then roughly 80 failures in 80 under eight concurrent copies, with CONTENT assertions failing, not connections. PID-derived port windows reduced the collision rate but could not eliminate it — the modulo still has birthdays, and commit review caught one failure in eight — so the final fix is deterministic between cooperating tests: an atomic temp-directory lease per port is held for the process lifetime, closing the probe-to-listen gap between concurrent copies. **Verified 80/80 under ten batches of eight concurrent processes.** Detection improved too: the test asserted `listen_ok` only at teardown, ~350 lines after the symptoms, so an early `RECORD(!listen_returned)` now names the cause first (a serving listen does not return, so a returned one plus a ready server means the responder is not ours), and one site that captured `listen_ok` and never checked it now does. **Not eliminated**: an unrelated process does not honor the lease and can still take the port — that is now loud rather than silent. | G8 requantization work | Resolved |
