...
- **mantic-mind** — cluster node; spawns and manages local `vllm serve` engine subprocesses; FTXUI status TUI
- **mantic-mind-control** — cluster head; manages nodes, agents, conversations, memories; FTXUI management TUI

> **Branch note:** this is the `vLLM-runtime` line. Agents use vLLM for local
> inference or an OpenAI-compatible remote API with `inference_backend = "api"`.
...
| Python + vLLM | on each node used for local inference — an existing `vllm` executable is used, or the node creates a managed install when `vllm_auto_provision = true` (the default) |
...
- **Load-aware routing.** Control scrapes each engine's Prometheus `/metrics`
  (running/waiting requests and KV-cache usage) and routes new agents to the
...
Image input is opt-in per agent. For a local multimodal model, enable vision on
the vLLM agent profile:
...
For vLLM and API-backed profiles, `vision_settings.enabled` declares that the
configured model accepts images. Image requests sent to a profile without
vision enabled fail with HTTP 422; Mantic-Mind never drops the images or
delegates the turn to another agent.

The profile editor exposes a Vision toggle. The Chat tab can stage up to eight
JPEG/PNG files, shows their sizes, uploads them through the same managed
attachment API used by other clients, and clears the staged list after the
request is queued.
...
| `vllm_server_path` | `MM_VLLM_PATH` | `vllm` | `vllm` CLI / wrapper used to launch local engines |
...
| `vllm_update_policy` | `MM_VLLM_UPDATE_POLICY` | `prompt` | Managed runtime update behavior: `prompt`, `auto`, or `manual` |
| `vllm_update_check` | `MM_VLLM_UPDATE_CHECK` | `true` | Periodically check the configured vLLM source for updates |
| `vllm_update_check_interval_hours` | `MM_VLLM_UPDATE_CHECK_INTERVAL_HOURS` | `24` | vLLM update-check interval |
...
```
POST   /api/node/load-model     { model_path, vision_enabled?, vllm_settings?, agent_id? }
...
POST   /api/node/restore-slot   { model_path, vision_enabled?, vllm_settings?, agent_id? }
